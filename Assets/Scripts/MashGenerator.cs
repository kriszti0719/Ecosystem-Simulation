...
public static class MeshGenerator
{
    ...
    public static MeshData GenerateTerrainMesh(float[,] heightMap, float hegihtMultiplier)
    {
        int width = heightMap.GetLength(0);
        int height = heightMap.GetLength(1);
        float topLeftX = (width - 1) / -2f;
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                meshData.vertices[vertexIndex] = new Vector3(topLeftX + x, heightMap[x, y] * hegihtMultiplier, topLeftZ - y);
